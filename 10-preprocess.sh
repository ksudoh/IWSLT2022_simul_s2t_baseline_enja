...
if test ! -e ${WORKDIR}/MuST-C/${SRC}-${TRG}/.done ; then
    env PYTHONPATH=${FAIRSEQ_ROOT} \
        ${python3} ${FAIRSEQ_ROOT}/examples/speech_to_text/prep_mustc_data_v2.py --data-root ${MUSTC_ROOT} --lang ${TRG} --output ${WORKDIR}/MuST-C --task asr st --vocab-type unigram --vocab-size-asr 8192 --vocab-size-st ${vocab_size_st} --cmvn-type global
    if test $? -eq 0 ; then
